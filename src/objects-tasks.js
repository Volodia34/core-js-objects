...
class CssSelector {
  ...
  checkOrder(order) {
    ...
    if (this.order.includes(orderMap[order])) {
      if (order === 'element' || order === 'id' || order === 'pseudoElement') {
        throw new Error(
    ...
    if (
      this.order.length > 0 &&
      this.order[this.order.length - 1] > orderMap[order]
    ) {
      throw new Error(
    ...
    this.order.push(orderMap[order]);
  }
